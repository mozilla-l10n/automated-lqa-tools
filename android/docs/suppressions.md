...
`android/locales/<code>/conventions.md` is prose, injected verbatim into
...
`android/locales/<code>/suppressions.yaml` is structured and applied after
...
```yaml
rules:
  - id: it-mozac-generic-wording
    reason: >-
      Shared android-components strings are deliberately generic; wording
      that reads oddly in one app is intentional.
    match:
      check: llm
      string_id: mozac_*
...
|---|---|
| `check` | which check raised it: `placeholders`, `selectors`, `plurals`, `escaping`, `markup`, `typography`, or `llm` |
