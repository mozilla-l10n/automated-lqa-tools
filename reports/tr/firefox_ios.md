...
- `DefaultBrowserCard.PeaceOfMind.Description.v108` — `tr/firefox-ios.xliff` — The "3,000+" (more than 3,000) figure is rendered as an exact 3.000, dropping the "+".
    ...
    - The source claims over 3,000 trackers; the translation states a flat 3,000, changing the product's own statistic claim.
...
- `Microsurvey.Prompt.TitleLabel.v127` — `tr/firefox-ios.xliff` — "It only takes a minute" is rendered as "takes only a few minutes".
    ...
    - The source says it takes only a minute; the Turkish says several minutes, overstating the time required.
...
### 🗑 Retired — the string no longer exists upstream (75)

- `CreditCard.SnackBar.RemoveCardSublabel.v112` — `Shared/Supporting Files/en.lproj/Alert.strings` — Turkish says "This card will be removed" instead of "This will remove the card from all of your synced devices", altering the subject.
...
- `Settings.AppIconSelection.AppIconNames.Twilight.Title.v137` — `Shared/Supporting Files/en.lproj/AppIconSelection.strings` — "Twilight" is translated as "Şafak" (dawn) instead of dusk/twilight.
    - Current: `Şafak`
    - Suggest: `Alacakaranlık`
...
- `Settings.AppIconSelection.SectionNames.Basics.Title.v139` — `Shared/Supporting Files/en.lproj/AppIconSelection.strings` — Section heading "Basics" rendered as the adjective "Basit" (simple) rather than a noun heading.
    - Current: `Basit`
    - Suggest: `Temel`
...
- `Settings.Home.Option.Stories.v140` — `Shared/Supporting Files/en.lproj/CustomizeFirefoxHome.strings` — "Stories" is translated as "Haberler" (News), which names a different thing than the source.
    - Current: `Haberler`
    - Suggest: `Hikâyeler`
...
- `Settings.Home.Option.TopStories.v143` — `Shared/Supporting Files/en.lproj/CustomizeFirefoxHome.strings` — "Top Stories" is rendered as "İlginç yazılar" (Interesting writings), which loses "Top" and conflicts with the other Stories strings in this screen.
    - Current: `İlginç yazılar`
...
- `Addresses.EditAddress.AutofillAddressDistrict.v129` — `Shared/Supporting Files/en.lproj/EditAddress.strings` — "District" and "County" are both translated as "İlçe" on the same address form, making the two distinct fields indistinguishable.
    - Current: `İlçe`
    - Suggest: `Semt`
...
- `Addresses.EditAddress.AutofillAddressProvince.v129` — `Shared/Supporting Files/en.lproj/EditAddress.strings` — "Province" is translated as "İl", which conflicts with the standard rendering and is inconsistent with the other administrative-division labels in the same form.
    - Current: `İl`
...
- `Engagement.Notification.Body.v112` — `Shared/Supporting Files/en.lproj/EngagementNotification.strings` — Turkish adds "restoranları" (restaurants), which is not in the source's generic "Find something nearby".
    - Current: `İster yakınınızdaki restoranları bulun, ister eğlenceli bir şeyler keşfedin.`
    - Suggest: `İster yakınınızdaki bir şeyi bulun, ister eğlenceli bir şeyler keşfedin.`
...
- `Engagement.Notification.Treatment.B.Body.v114` — `Shared/Supporting Files/en.lproj/EngagementNotification.strings` — Uses the informal singular imperative "gezin" while the sibling notification strings use the polite plural form ("gezinin", "deneyin", "bulun").
    - Current: `%@ ile çerezleriniz ve geçmişiniz kaydedilmeden gezin.`
    - Suggest: `%@ ile çerezleriniz ve geçmişiniz kaydedilmeden gezinin.`
...
- `CloseTab.ArrivingNotification.title.v133` — `Shared/Supporting Files/en.lproj/FxANotification.strings` — The Turkish reads "Firefox's tabs closed" instead of "Firefox tabs closed: <count>", and the possessive suffix on the app name is wrong for the intended meaning.
...
- `MainMenu.AccessibilityLabels.DismissBanner.142` — `Shared/Supporting Files/en.lproj/MainMenu.strings` — "banner" is translated as "Bildirim" (notification) instead of banner.
    - Current: `Bildirimi kapat`
    - Suggest: `Banner’ı kapat`
...
- `MainMenu.SettingsSection.AccessibilityLabels.WhatsNew.v132` — `Shared/Supporting Files/en.lproj/MainMenu.strings` — Wrong apostrophe suffix for "Firefox": should be "%@'taki" → correct vowel harmony gives "Firefox'taki", but the placeholder form used is inconsistent with app name ending.
...
- `MainMenu.SettingsSection.WhatsNew.Title.v131` — `Shared/Supporting Files/en.lproj/MainMenu.strings` — Wrong Turkish suffix after the app-name placeholder: "%@’taki" should be "%@’teki".
    - Current: `%@’taki yenilikler`
    - Suggest: `%@’teki yenilikler`
...
- `Microsurvey.Prompt.TitleLabel.v127` — `Shared/Supporting Files/en.lproj/Microsurvey.strings` — "It only takes a minute" is translated as "birkaç dakika" (a few minutes) instead of one minute.
    - Current: `Yalnızca birkaç dakika sürer.`
    - Suggest: `Yalnızca bir dakikanızı alır.`
    - The source says the survey takes only a minute; the Turkish says several minutes, overstating the time required.
- `NativeErrorPage.ButtonLabel.v131` — `Shared/Supporting Files/en.lproj/NativeErrorPage.strings` — "Reload" is rendered as "Tazele" instead of the standard Firefox Turkish term "Yeniden yükle".
    - Current: `Tazele`
    - Suggest: `Yeniden yükle`
...
- `NativeErrorPage.CellularDataRestricted.Description.v156` — `Shared/Supporting Files/en.lproj/NativeErrorPage.strings` — "Wi-Fi’a" uses the wrong vowel-harmony suffix; Turkish convention is "Wi-Fi’ye".
    - Current: `Wi-Fi’a bağlanın`
    - Suggest: `Wi-Fi’ye bağlanın`
    - "Wi-Fi" ends in a front vowel (i), so the dative suffix must be -ye, not -a.
- `DefaultBrowserPopup.DescriptionFooter.v124` — `Shared/Supporting Files/en.lproj/Onboarding.strings` — Informal second-person singular "dokun" breaks the formal address used in the surrounding Default Browser Popup strings.
    - Current: `Bu iletiyi kapatıp Atla’ya dokun.`
    - Suggest: `Bu iletiyi kapatıp Atla’ya dokunun.`
...
- `Onboarding.Modern.BrandRefresh.TermsOfUse.ManagePreferenceAgreement.v148` — `Shared/Supporting Files/en.lproj/Onboarding.strings` — "the browser" is rendered as "uygulamayı" (the app) instead of "tarayıcıyı".
    - Current: `uygulamayı geliştirmemize`
    - Suggest: `tarayıcıyı geliştirmemize`
    - The source says "To help improve the browser"; the Turkish says "the app", which is a different term than the source's "browser".
- `Onboarding.Modern.TermsOfService.AgreementButtonTitle.v145` — `Shared/Supporting Files/en.lproj/Onboarding.strings` — "Continue" rendered as "İleri" (Next), inconsistent with the v140 string on the same screen which uses "Devam et".
    - Current: `İleri`
    - Suggest: `Devam et`
...
- `Onboarding.Modern.TermsOfService.ManagePreferenceAgreement.v145` — `Shared/Supporting Files/en.lproj/Onboarding.strings` — "the browser" translated as "uygulamayı" (the app) instead of "tarayıcıyı".
    - Current: `uygulamayı geliştirmemize`
    - Suggest: `tarayıcıyı geliştirmemize`
    - Source says "To help improve the browser"; the v140 counterpart correctly uses "tarayıcıyı".
- `PasswordAutofill.SignInWithSavedPassword.v124` — `Shared/Supporting Files/en.lproj/PasswordAutofill.strings` — Future/intent "You'll sign into %@" is rendered as present continuous "you are signing in".
    - Current: `%@ sitesine giriş yapıyorsunuz`
    - Suggest: `%@ sitesine giriş yapacaksınız`
    - The en-US says the user will sign in (future), whereas the Turkish states the action is currently happening.
- `UnifiedSearch.SearchEngineSelection.AccessibilityLabels.TopTitle.Label.v133` — `Shared/Supporting Files/en.lproj/SearchEngineSelection.strings` — Accessibility label for "This time search in:" omits the "this time" meaning.
    - Current: `Burada ara:`
    - Suggest: `Bu kez şurada ara:`
    - Same as the visible title: the one-off nature of the engine selection ("This time") is not conveyed.
- `UnifiedSearch.SearchEngineSelection.TopTitle.Title.v133` — `Shared/Supporting Files/en.lproj/SearchEngineSelection.strings` — "This time search in:" is translated as "Burada ara:" (Search here), losing the "this time" meaning.
    - Current: `Burada ara:`
    - Suggest: `Bu kez şurada ara:`
    - The source emphasizes a one-time engine choice ("This time"); the Turkish only says "Search here", dropping that meaning.
- `Search.Google.Title.v108` — `Shared/Supporting Files/en.lproj/SearchHeaderTitle.strings` — "Google Search" section header translated as an imperative "Google’da Ara" instead of a noun phrase consistent with the sibling header "%@ araması".
    - Current: `Google’da Ara`
    - Suggest: `Google araması`
...
- `Settings.SearchZero.TrendingSearches.Toggle.v146` — `Shared/Supporting Files/en.lproj/SearchZero.strings` — "Trending Searches" rendered as "Arama trendleri" here but as "gündeminde" elsewhere; inconsistent with the section title wording.
    - Current: `Arama trendlerini göster`
...
- `Addresses.Settings.SecureSaveInfo.Description.v130` — `Shared/Supporting Files/en.lproj/Settings.strings` — Case mismatch: "Bilgilerinizi ... hızlıca erişmek" requires the dative "bilgilerinize" for the verb "erişmek".
...
- `Settings.AIControls.AIPoweredFeaturesSection.BlockedStatusDescriptionV2.v151` — `Shared/Supporting Files/en.lproj/Settings.strings` — The condition "For on-device AI" is dropped, turning a conditional statement into an unconditional one.
...
- `Settings.DailyUsagePing.Message.v135` — `Shared/Supporting Files/en.lproj/Settings.strings` — The placeholder (company name, e.g. Mozilla) is misplaced so the text reads "active Mozilla users" instead of "helps Mozilla estimate active users".
...
- `Settings.Notifications.TipsAndFeaturesNotificationsStatus.v112` — `Shared/Supporting Files/en.lproj/Settings.strings` — The second clause "and how to get the most out of %@" is dropped from the translation.
...
- `Settings.Rollouts.Message.v148` — `Shared/Supporting Files/en.lproj/Settings.strings` — "between updates" is mistranslated as "with every update" (her güncellemede).
...
- `Settings.Search.Suggest.ShowNonSponsoredSuggestions.Description.v124.v2` — `Shared/Supporting Files/en.lproj/Settings.strings` — Wrong apostrophe suffix vowel for the app name; Turkish requires "-tan" only after back vowels, and "Firefox" takes "-tan" but the placeholder is separated incorrectly with the wrong harmony form.
    - Current: `%@’tan`
    - Suggest: `%@’ten`
...
- `SentFromFirefox.SocialShare.ShareMessageA.Title.v134` — `Shared/Supporting Files/en.lproj/SocialShare.strings` — "Sent from %2$@" is translated as "paylaşıldı" (shared) instead of "gönderildi" (sent).
...
- `SentFromFirefox.SocialShare.ShareMessageA.Title.v137` — `Shared/Supporting Files/en.lproj/SocialShare.strings` — "Sent from %2$@" is translated as "paylaşıldı" (shared) instead of "gönderildi" (sent).
    - Current: `%2$@ ile paylaşıldı`
    - Suggest: `%2$@ ile gönderildi`
    - The source says the link was sent from Firefox; "paylaşıldı" means "was shared", changing the meaning.
- `SentFromFirefox.SocialShare.ShareMessageB.Title.v134` — `Shared/Supporting Files/en.lproj/SocialShare.strings` — "Sent from %2$@" is translated as "paylaşıldı" (shared) instead of "gönderildi" (sent).
    - Current: `%2$@ ile paylaşıldı`
    - Suggest: `%2$@ ile gönderildi`
    - The source says the link was sent from Firefox; "paylaşıldı" means "was shared", changing the meaning.
- `SentFromFirefox.SocialShare.ShareMessageB.Title.v137` — `Shared/Supporting Files/en.lproj/SocialShare.strings` — "Sent from %2$@" is translated as "paylaşıldı" (shared) instead of "gönderildi" (sent).
    - Current: `%2$@ ile paylaşıldı`
    - Suggest: `%2$@ ile gönderildi`
    - The source says the link was sent from Firefox; "paylaşıldı" means "was shared", changing the meaning.
- `Summarizer.Error.UnsafeWebsite.Message.v142` — `Shared/Supporting Files/en.lproj/Summarizer.strings` — "This page may be restricted" is rendered with an added "içerik içeriyor" construction that mixes the two clauses awkwardly and misstates the meaning.
...
- `Summarizer.ToastLabel.v149` — `Shared/Supporting Files/en.lproj/Summarizer.strings` — "Summary not available" translated with the Arabic-origin "mevcut değil" where standard Firefox tr wording is "kullanılamıyor"/"yok".
...
- `TabTray.TabsSelectorSyncedTabsTitle.v140` — `Shared/Supporting Files/en.lproj/TabsTray.strings` — "Sync" is translated as the adjective/participle "Eşitlenen" ("synced") instead of the noun "Eşitleme".
    - Current: `Eşitlenen`
    - Suggest: `Eşitleme`
    - Source is the noun "Sync" as a tab-selector title; "Eşitlenen" is a dangling participle ("the synced ...") without a head noun.
- `TabTrayOneDayAgoTitle.v140` — `Shared/Supporting Files/en.lproj/TabsTray.strings` — "1 Day Ago" is rendered as "1 gün" ("1 day"), dropping the "ago" relative-time meaning.
    - Current: `1 gün`
    - Suggest: `1 gün önce`
...
- `TabTrayOneMonthAgoTitle.v140` — `Shared/Supporting Files/en.lproj/TabsTray.strings` — "1 Month Ago" is rendered as "1 ay" ("1 month"), dropping "ago".
    - Current: `1 ay`
    - Suggest: `1 ay önce`
    - The label denotes a relative time point ("ago"); the Turkish only says "1 month".
- `TabTrayOneWeekAgoTitle.v140` — `Shared/Supporting Files/en.lproj/TabsTray.strings` — "1 Week Ago" is rendered as "1 hafta" ("1 week"), dropping "ago".
    - Current: `1 hafta`
    - Suggest: `1 hafta önce`
    - The label denotes a relative time point ("ago"); the Turkish only says "1 week".
- `TermsOfUse.Title.v142` — `Shared/Supporting Files/en.lproj/TermsOfUse.strings` — "We’ve got an update" is translated as "Bir haberimiz var" ("We have some news"), losing the notion of an update.
...
- `WebCompatReporter.AdditionalInfo.FooterText.v154` — `Shared/Supporting Files/en.lproj/WebCompatReporter.strings` — Ungrammatical double structure "iyileştirmemize için yardımcı" (superfluous "için" after a dative verbal noun).
...
- `WebCompatReporter.Preview.Data.TrackingProtectionSetting.v155` — `Shared/Supporting Files/en.lproj/WebCompatReporter.strings` — "Enhanced Tracking Protection" is rendered as "gelişmiş izlenme koruması" instead of the established Firefox term "Gelişmiş İzlenme Koruması".
...
- `WebCompatReporter.SubOption.CaptionsMissing.v154` — `Shared/Supporting Files/en.lproj/WebCompatReporter.strings` — "Captions are missing" translated as "Altyazılar görünmüyor" (captions are not visible) instead of missing.
...
- `WorldCup.GroupPhase.GroupStageLabel.v151` — `Shared/Supporting Files/en.lproj/WorldCup.strings` — "Group Stage" (tournament phase) is rendered as "Grup maçı" (group match), naming a single match rather than the phase.
    - Current: `Grup maçı`
...
- `WorldCup.HomepageWidget.GetCustomWallpaperLabel.v151` — `Shared/Supporting Files/en.lproj/WorldCup.strings` — "Get custom wallpaper" is translated as "Kişisel duvar kâğıdını indir" (download the personal wallpaper), altering the meaning with a definite object and "download".
...
- `WorldCup.HomepageWidget.RoundPhase.Round16Label.v151` — `Shared/Supporting Files/en.lproj/WorldCup.strings` — Inconsistent/incorrect apostrophe suffix casing: "16’LI TUR" should follow vowel harmony as "16’LI" vs the parallel "32’Lİ" — the correct form for 16 (on altı) is "16’LI"? see rationale.
    - Current: `16’LI TUR`
    - Suggest: `SON 16 TURU`
...
- `Enter passcode` — `Shared/en.lproj/AuthenticationManager.strings` — "passcode" translated as "Parola" (password) instead of "parola kodu/geçiş kodu".
    - Current: `Parolayı girin`
    - Suggest: `Geçiş kodunu girin`
...
- `This action will clear all of your private data, including history from your synced devices.` — `Shared/en.lproj/ClearHistoryConfirm.strings` — "private data" rendered as "kişisel verileriniz" (personal data) instead of "özel verileriniz".
...
- `Saved Logins` — `Shared/en.lproj/ClearPrivateData.strings` — "Saved Logins" is rendered as "Kayıtlı hesaplar" (saved accounts) instead of saved logins/passwords.
...
- `DefaultBrowserCard.BetterInternet.Title.v108` — `Shared/en.lproj/Default Browser.strings` — "Default to a Better Internet" is translated as "Hep daha iyi bir internet" ("Always a better internet"), losing the call to set the default browser.
...
- `DefaultBrowserCard.PeaceOfMind.Description.v108` — `Shared/en.lproj/Default Browser.strings` — "3,000+ trackers" is translated as "3.000 takip kodunu", dropping the "+" (more than).
    - Current: `ortalama 3.000 takip kodunu engelliyor`
    - Suggest: `ortalama 3.000’den fazla takip kodunu engelliyor`
    - The en-US states over 3,000 trackers per user per month; the Turkish states exactly 3,000, changing the factual claim.
- `AddPass.Error.Message` — `Shared/en.lproj/Localizable.strings` — "Pass" (Wallet kartı/bileti) is mistranslated as "Parola" (password).
    - Current: `Parola Wallet’a eklenirken bir hata oluştu.`
    - Suggest: `Kart Wallet’a eklenirken bir hata oluştu.`
    - The developer comment references Apple Wallet passes; "pass" is a Wallet card/ticket, not a password ("parola").
- `AddPass.Error.Title` — `Shared/en.lproj/Localizable.strings` — "Pass" (Wallet kartı) mistranslated as "Parola" (password).
    - Current: `Parola eklenemedi`
    - Suggest: `Kart eklenemedi`
    - Per the developer comment this is the Apple Wallet 'Add Pass Failed' alert; a Wallet pass is not a password.
- `Logins` — `Shared/en.lproj/Localizable.strings` — "Logins" is rendered as "Hesaplar" (Accounts), which is the wrong term for saved logins/passwords.
    - Current: `Hesaplar`
    - Suggest: `Hesap bilgileri`
...
- `Logins.PasscodeRequirement.Warning` — `Shared/en.lproj/Localizable.strings` — The Turkish says the user must "set" a device passcode rather than that a passcode must be enabled, and drops "for Firefox" as a separate notion by turning it into a possessive.
...
- `Menu.RemovePin.Confirm2.v99` — `Shared/en.lproj/Localizable.strings` — "Shortcuts" is rendered as "Kestirmeler" here but as "Kısayollar" in the related shortcut string on the same screen.
...
- `Oops! Firefox crashed` — `Shared/en.lproj/Localizable.strings` — The interjection "Oops!" is dropped from the translation.
...
- `Privacy` — `Shared/en.lproj/Localizable.strings` — Section title is written in all caps unlike the source and other section titles.
    - Current: `GİZLİLİK`
    - Suggest: `Gizlilik`
