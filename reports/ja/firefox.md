...
- `autofill-address-family-name` — `browser/browser/preferences/formAutofill.ftl` — surname field labelled 氏; the standard pair is 姓/名, and autofill-address-given-name is already 名. certificate-viewer-surname also uses 姓. → 姓 Verified.
  - Current: `姓`
  - Source: `Last Name`
  - (Retired: the suggested text is identical to the current text.)

### 🔁 String changed, defect not verifiable — needs a re-read (80)
